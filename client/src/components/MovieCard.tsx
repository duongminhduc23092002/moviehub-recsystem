...
export default function MovieCard({ movie, id, title, poster, year, rating, ratingsCount }: Props) {
  ...
  const movieRatingsCount = movie?.ratingsCount || ratingsCount || 0;
    ...
    >
      ...
      <div className="relative aspect-[2/3] bg-netflix-gray overflow-hidden rounded-lg
        ...
        <div className="absolute top-3 left-3 flex items-center gap-1.5
                      bg-netflix-black/90 backdrop-blur-sm px-3 py-1.5 rounded-full
          ...
          <span className="text-white font-bold text-sm">
            {movieRating && movieRating > 0 ? movieRating.toFixed(1) : "N/A"}
          </span>
          <span className="text-netflix-light text-xs">
            /10
          </span>
          {movieRatingsCount > 0 && (
            <span className="text-netflix-light text-xs ml-1">
              ({movieRatingsCount.toLocaleString()})
            </span>
          )}
        </div>
